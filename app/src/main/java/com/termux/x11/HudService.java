...
public class HudService extends Service {
    ...
    private void startFpsReaderThread() {
        ...
        fpsReaderThread = new Thread(() -> {
            ...
            try {
                // Run logcat with clear and format, but we just need lines containing LorieNative
                process = Runtime.getRuntime().exec(new String[]{"logcat", "-v", "brief"});
                ...
                while (fpsReaderRunning && (line = reader.readLine()) != null) {
                    if (line.contains("LorieNative") && line.contains("FPS")) {
                        // Example: "02-16 17:45:28.796  5545  5570 I LorieNative: 34 frames in 5.0 seconds = 6.8 FPS"
                        // Try to extract the number after '='
                        int idx = line.lastIndexOf('=');
                        if (idx != -1) {
                            String afterEq = line.substring(idx + 1).trim();
                            // afterEq might be "6.8 FPS" or "6.8"
                            String[] parts = afterEq.split("\\s+");
                            if (parts.length > 0) {
                                String fpsNum = parts[0];
                                fpsValue = "FPS: " + fpsNum;
                            }
                        } else {
                            // Fallback: look for a number before "FPS"
